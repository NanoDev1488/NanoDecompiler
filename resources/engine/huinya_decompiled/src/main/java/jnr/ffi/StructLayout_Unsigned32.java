// исходный (обфусцированный) внутренний класс: jnr.ffi.StructLayout.Unsigned32
package jnr.ffi;

import jnr.ffi.NativeType;
import jnr.ffi.Pointer;
import jnr.ffi.StructLayout;
import jnr.ffi.StructLayout_NumberField;
import jnr.ffi.StructLayout_Offset;

public class StructLayout_Unsigned32 extends StructLayout_NumberField {

    // ---- поля ----
  final StructLayout this$0;

  public StructLayout_Unsigned32(StructLayout arg0) { // было: <init>
        super(arg0, NativeType.UINT);
        this$0 = arg0;
    }

  public StructLayout_Unsigned32(StructLayout arg0, StructLayout_Offset arg1) { // было: <init>
        super(arg0, NativeType.SINT, arg1);
        this$0 = arg0;
    }

  public final long get(Pointer arg0) {
        long var2 = ((long) arg0.getInt(offset()));
        return var2 >= 0L ? var2 : (var2 & 2147483647L) + 2147483648L;
    }

  public final void set(Pointer arg0, long arg1) {
        arg0.putInt(offset(), ((int) arg1));
    }

  public void set(Pointer arg0, Number arg1) {
        arg0.putInt(offset(), arg1.intValue());
    }

  public final int intValue(Pointer arg0) {
        return ((int) get(arg0));
    }

  public final long longValue(Pointer arg0) {
        return get(arg0);
    }

  public String toString(Pointer arg0) {
        return super.toString(arg0);
    }

  public short shortValue(Pointer arg0) {
        return super.shortValue(arg0);
    }

  public byte byteValue(Pointer arg0) {
        return super.byteValue(arg0);
    }

  public float floatValue(Pointer arg0) {
        return super.floatValue(arg0);
    }

  public double doubleValue(Pointer arg0) {
        return super.doubleValue(arg0);
    }

}
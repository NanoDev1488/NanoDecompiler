...
// БАГ-ФИКС v1.7.4 (реальный краш на Windows - std::filesystem::filesystem_error
// "Cannot convert character sequence: Illegal byte sequence", падал сразу
// на старте с реальным jar): fs::path(std::string) на MinGW/Windows
// конвертирует narrow-строку в wchar_t через ТЕКУЩУЮ СИСТЕМНУЮ КОДОВУЮ
// СТРАНИЦУ (ACP), а НЕ через UTF-8 - если строка (имя записи в jar с
// нестандартной кодировкой, ПУТЬ ПОЛЬЗОВАТЕЛЯ с кириллическим именем
// Windows-профиля и т.п.) содержит байты, которые не образуют валидную
// последовательность в этой кодовой странице - конвертация БРОСАЕТ
// исключение вместо тихого искажения текста. У нас ВЕЗДЕ строки в UTF-8
// по конвенции проекта - fs::u8path() вместо fs::path() гарантированно
// трактует байты как UTF-8 независимо от системной локали/кодовой
...
JarProcessResult process_jar_with_stats(const std::string& jar_path, const std::string& out_dir, bool skip_legitimacy, bool print_progress) {
    ...
    // НОВОЕ v1.8.0 (реальный запрос - подписи стадий вместо голого "%",
    // а заодно честная причина "прогресс два раза доходит до 100%":
    // раньше GUI ничего не знал о фазах ДО начала рендеринга классов
    // (валидация/парсинг/проверка легитимности - может занимать секунды
    // на сетевых запросах) и подтягивал процент время-экстраполяцией
    // почти до упора, а когда стартовал НАСТОЯЩИЙ прогресс рендеринга -
    // выглядело как "второй проход". Печатаем маркер стадии - GUI сам
    // решает, как его показать (см. classifyLine/parseEngineStage в
    // engine.tsx) - здесь только сырой, легко парсимый текст.
    ...
    for (auto& n : all_names) {
        ...
        // ТОЛЬКО если это имя принадлежит ИЗВЕСТНОЙ бандленной библиотеке
        // (та же таблица known_libs(), что и для .pom/relocations) -
        // значит регистрацию сервиса написал автор библиотеки, а не автор
        // плагина, и она станет доступна заново через сам depndency в
        // pom.xml при сборке. Если префикс НЕ совпал ни с одной известной
        // библиотекой - оставляем файл (может быть собственный сервис
        // самого плагина, удалять вслепую рискованнее, чем оставить один
        // лишний файл в выводе).
        if (n.rfind("META-INF/services/", 0) == 0) {
            std::string service_name = n.substr(18);  // strlen("META-INF/services/") == 18
            bool is_library_service = false;
            for (auto& lib : nd::known_libs())
                if (service_name.rfind(lib.prefix, 0) == 0) {
                    is_library_service = true;
                    break;
                }
            if (is_library_service) continue;
        }
        bool skip = false;

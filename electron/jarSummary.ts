 ...
 * напрямую через central directory ZIP-формата, БЕЗ спавна Python-
 * процесса. См. HANDOFF_22: раньше каждый выбор jar в UI спавнил
 ...
 * Здесь читаются только МЕТАДАННЫЕ central directory (имена файлов,
 * размеры, смещения) - без распаковки содержимого, КРОМЕ двух маленьких
 * файлов (первый .class - 8 байт заголовка для версии Java, plugin.yml
 * целиком - обычно первые сотни байт) - на реальных jar это исполняется
 * за единицы миллисекунд, а не сотни.
 *
 ...
 * через Python-подпроцесс (см. jarSummaryFallback ниже) - корректность
 * важнее скорости на редком edge-case.
 */
...
/** Та же формула, что и java_version_from_major() в gui_common.py -
